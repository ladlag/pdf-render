...
**Note**: The library also supports a legacy 4-section structure for backward compatibility:
...
For backward compatibility, the library still supports the legacy structure with 4 predefined sections:

```java
import com.mercury.pdf.render.model.*;
import java.util.Arrays;

// Legacy structure with 4 predefined sections
ReportData report = new ReportData();
report.setTitle("Legacy Format Report");
report.setReportDate("2024-12-31");

// Section 1: Detailed tables with grouped blocks
TableBlock block1 = new TableBlock("1.1", "Analysis Block 1", tableData);
report.setTableBlocks(Arrays.asList(block1));

// Section 2: Analysis paragraphs
report.setAnalysisParagraphs(Arrays.asList("Paragraph 1", "Paragraph 2"));
...
// Section 4: Notice and metadata
report.setReportNotice("Confidential information");
report.setMetadata("Generated by PDF Render");
```

**Note:** The legacy structure is maintained for backward compatibility. New projects should use the flexible section-based approach.
